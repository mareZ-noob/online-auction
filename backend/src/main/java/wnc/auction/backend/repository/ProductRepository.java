...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    Page<Product> findByCategoryAndActive(
            @Param("categoryId") Long categoryId, @Param("now") LocalDateTime now, Pageable pageable);

    // Full-text search
    @Query("SELECT p FROM Product p WHERE " + "p.status = 'ACTIVE' AND p.endTime > :now AND "
            + "(LOWER(p.name) LIKE LOWER(CONCAT('%', :keyword, '%')) OR "
            + "LOWER(p.description) LIKE LOWER(CONCAT('%', :keyword, '%')))")
    Page<Product> searchProducts(@Param("keyword") String keyword, @Param("now") LocalDateTime now, Pageable pageable);

    // Search by category and keyword
    @Query("SELECT p FROM Product p WHERE " + "p.category.id IN :categoryIds AND "
            + "p.status = 'ACTIVE' AND p.endTime > :now")
    Page<Product> findByCategoryIdsAndActive(
            @Param("categoryIds") List<Long> categoryIds, @Param("now") LocalDateTime now, Pageable pageable);
    ...
    @Query("SELECT p FROM Product p WHERE " + "p.category.id IN :categoryIds AND "
            ...
            + "(LOWER(p.name) LIKE LOWER(CONCAT('%', :keyword, '%')) OR "
            + "LOWER(p.description) LIKE LOWER(CONCAT('%', :keyword, '%')))")
    Page<Product> searchProductsByCategoryIds(
    ...
    // Top 5 ending soon
    @Query("SELECT p FROM Product p WHERE " + "p.status = 'ACTIVE' AND p.endTime > :now " + "ORDER BY p.endTime ASC")
    List<Product> findTop5EndingSoon(@Param("now") LocalDateTime now, Pageable pageable);

    // Top 5 most bids
    @Query("SELECT p FROM Product p WHERE " + "p.status = 'ACTIVE' AND p.endTime > :now " + "ORDER BY p.bidCount DESC")
    ...
    @Query("SELECT p FROM Product p WHERE " + "p.seller.id = :sellerId AND "
            + "p.status = 'ACTIVE' AND p.endTime > :now")
    Page<Product> findBySellerActive(
            @Param("sellerId") Long sellerId, @Param("now") LocalDateTime now, Pageable pageable);

    // Products won by bidder
    @Query("SELECT p FROM Product p WHERE " + "p.currentBidder.id = :bidderId AND "
            + "p.status = 'COMPLETED' AND p.endTime < :now")
    Page<Product> findProductsWonByBidder(
            @Param("bidderId") Long bidderId, @Param("now") LocalDateTime now, Pageable pageable);

    // Products bidded by user
    @Query("SELECT DISTINCT p FROM Product p " + "JOIN p.bids b WHERE b.user.id = :userId AND "
            + "p.status = 'ACTIVE' AND p.endTime > :now")
    Page<Product> findProductsBiddedByUser(
    ...
    List<Product> findProductsForAutoExtend(
