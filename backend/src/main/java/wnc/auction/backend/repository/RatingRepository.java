...
public interface RatingRepository extends JpaRepository<Rating, Long> {
    ...
    Optional<Rating> findByUserIdAndRatedByIdAndProductId(Long userId,
                                                          Long ratedById,
                                                          Long productId);

    @Query("SELECT COUNT(r) FROM Rating r WHERE " +
            "r.user.id = :userId AND r.isPositive = true")
    int countPositiveRatings(@Param("userId") Long userId);

    @Query("SELECT COUNT(r) FROM Rating r WHERE " +
            "r.user.id = :userId AND r.isPositive = false")
