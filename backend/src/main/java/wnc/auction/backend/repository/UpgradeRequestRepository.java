...
public interface UpgradeRequestRepository extends JpaRepository<UpgradeRequest, Long> {
    ...
    @Query("SELECT ur FROM UpgradeRequest ur WHERE ur.status = 'PENDING' " + "ORDER BY ur.createdAt ASC")
    Page<UpgradeRequest> findPendingRequests(Pageable pageable);

    @Query("SELECT ur FROM UpgradeRequest ur WHERE ur.user.id = :userId " + "ORDER BY ur.createdAt DESC")
    List<UpgradeRequest> findByUserId(@Param("userId") Long userId);

    @Query("SELECT ur FROM UpgradeRequest ur WHERE ur.user.id = :userId " + "AND ur.status = 'PENDING'")
